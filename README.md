...
`master` (this branch) is the baseline flavor and the common base the others build on.
The body of this README documents the `master` lineage; per-flavor specifics live in
each branch's [`docs/learning/`](docs/learning/).
...
3. **Throwaway spikes stay throwaway** — merge or delete promptly; don't let dead
   experiments accumulate as branches.
